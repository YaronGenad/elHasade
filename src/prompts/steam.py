...
def build_stem_comprehension_prompt(subject: str, topic: str, grade: str,
    ...
    if input_type == 'observation':
        obs_table_schema = """,
  "observation_table": {
    "title": "טבלת תצפית מונחת",
    "columns": ["מה צפיתי", "מה שמתי לב אליו", "שאלה שעלתה"],
    "rows": 5
  }"""
...
{{
  ...
  "discussion_starters": [
    ...
    "שאלת השוואה: מה דומה? מה שונה? מה ניתן להסיק?",
...
{{
  ...
  "graph_scaffold": {{
    "title": "כותרת הגרף",
    "x_axis_label": "ציר X — המשתנה שנשנה (המשתנה הבלתי-תלוי)",
    "y_axis_label": "ציר Y — מה מדדנו (המשתנה התלוי)",
    "note": "ציירו/י את הגרף — רשמו נקודות ו/או עמודות לפי הנתונים מהטבלה"
  }},
  "self_assessment": [
    "הצלחתי ל: ___",
    "עדיין קשה לי: ___",
...
**פעילויות יצירה לסיימים מוקדם (בחר אחת ל-extension_activity):**
בניית לוח מושגים מדעי (Word Wall STEAM), פיסול מושג בפלסטלינה (אטום, תא, גל),
יצירת כרטיסיות מושגים אישיות עם ציור, בניית ספר מושגים קטן, עיצוב פוסטר מושגים, יצירת ציר זמן מדעי-היסטורי
...
{{
  ...
  "extension_activity": {{
    "title": "המשך ואתגר — לסיימים מוקדם",
    "instruction": "הוראה קצרה לפעילות היצירה (ציין מה לעשות עם המושגים)",
    "items": ["פעילות יצירה ספציפית מהרשימה המתאימה לנושא ולגיל", "פעילות יצירה נוספת (אופציונלי)"]
  }}
}}"""
